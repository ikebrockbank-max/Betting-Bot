...
def grade(r):
    ...
    if r.get("sport") == "MLB":
        a = _fetch_actual_mlb(r["player"], r["stat_type"], date)
...
buckets={"STAR":[], "gate":[], "all":[]}
print(f"\n{'':2}{'tier':5}{'player':<22}{'pick':<34}{'actual':>7}")
for r in sorted(rows, key=lambda x:(not _is_elite(x), -f(x,"confidence"))):
    hit,a = grade(r)
    tier = "STAR" if _is_elite(r) else ("gate" if (r.get("sport")=="MLB" and _passes_direction_gate(r)) else "")
    mark = "⬜" if hit is None else ("✅" if hit else "❌")
    pick=f"{r['direction']} {r['line']} {r['stat_type']}"
    if _is_elite(r) or (r.get("sport")=="MLB" and _passes_direction_gate(r) and f(r,'confidence')>=0.7):
        print(f"{mark} {tier:5}{r['player']:<22}{pick:<34}{str(a):>7}")
    if hit is not None:
        buckets["all"].append(hit)
        if _is_elite(r): buckets["STAR"].append(hit)
        if r.get("sport")=="MLB" and _passes_direction_gate(r): buckets["gate"].append(hit)
print()
for k in ("STAR","gate","all"):
    b=buckets[k]
